<div class="min-h-screen bg-stone-100 p-4 lg:p-6 font-['Jost']">
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 max-w-[1600px] mx-auto">
        ...
        <div class="lg:col-span-8 flex flex-col space-y-6">
            ...
            <!-- Pencarian & Header -->
            <div class="bg-white p-4 rounded-2xl shadow-sm border border-stone-200 flex items-center space-x-4" x-data="{ showScanner: false }" @close-modal.window="showScanner = false">
                <div class="flex-1 relative">
                    ...
                    <input type="text" 
                        ...
                        class="w-full pl-10 pr-10 py-3 bg-stone-50 border-stone-200 rounded-xl focus:ring-amber-500 focus:border-amber-500 transition-all text-stone-700"
                    >
                    @if($search)
                        <button wire:click="$set('search', '')" class="absolute inset-y-0 right-0 pr-3 flex items-center text-stone-400 hover:text-stone-700 transition-colors cursor-pointer">
                ...
                <button @click="showScanner = true" 
                        class="p-3 bg-stone-900 text-white rounded-xl hover:bg-stone-800 transition-all shadow-lg shadow-stone-200/50 group" 
                        title="Scan Barcode">
                ...
                <div x-show="showScanner" 
                     ...
                     class="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-stone-900/60 backdrop-blur-sm" 
                    ...
                    <div class="bg-white rounded-[2rem] shadow-2xl w-full max-w-lg overflow-hidden border border-stone-200" 
                        ...
                        <div class="px-8 py-5 flex items-center justify-between border-b border-stone-100">
                            <div>
                                <h3 class="text-lg font-black text-stone-900">Scan Barcode</h3>
                                <p class="text-[10px] text-stone-400 font-bold uppercase tracking-widest mt-0.5">
                                    Scan → Otomatis Masuk Keranjang
                                </p>
                            </div>
                            <button @click="showScanner = false" class="p-2 text-stone-400 hover:text-stone-900">
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4 overflow-y-auto max-h-[calc(100vh-220px)] pr-2">
                ...
                @empty
                    <div class="col-span-full py-20 flex flex-col items-center justify-center text-stone-400">
                        ...
                        @if($search)
                            <p class="text-stone-500 font-medium">Tidak ada produk untuk "<span class="font-bold text-stone-700">{{ $search }}</span>"</p>
                            <button wire:click="$set('search', '')" class="mt-2 text-xs font-bold text-amber-600 hover:text-amber-700 cursor-pointer uppercase tracking-widest">Hapus Pencarian</button>
                        @else
                            <p class="text-stone-500 font-medium">Belum ada produk aktif</p>
                        @endif
                    </div>
                @endforelse
            </div>
